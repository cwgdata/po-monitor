...
def list_schemas(catalog: str, client=Depends(_user_client)):
    try:
        result = execute_sql(client, f"SHOW SCHEMAS IN `{catalog}`")
        rows = rows_as_dicts(result)
...
def list_tables(
    ...
    """
    try:
        show = execute_sql(client, f"SHOW TABLES IN `{catalog}`.`{schema}`")
        ...
        for n in names:
            try:
                d = execute_sql(client, f"DESCRIBE EXTENDED `{catalog}`.`{schema}`.`{n}`")
                fmt = None
                table_type = None
                for row in d["rows"]:
...
def diag(
    ...
    out = {
        ...
        "token_prefix": x_forwarded_access_token[:20] if x_forwarded_access_token else None,
